...
int16_t unk_3920; // тип указан от фонаря
//_UNKNOWN checktype; // weak
int16_t checktype; // тип указан от фонаря

//_UNKNOWN g_type; // weak
int16_t g_type; // тип указан от фонаря
        ...
        */
        ...
        if (v7 != 1 || (uint8_t) s[0] != 255) {
            ...
            if (v16 <= 0) {
                LABEL_26:
                close(v3);
                __android_log_print(4, "ScannerJNI",
...
jint Java_android_hardware_barcode_Scanner_ReadDataSCA(JNIEnv *env, jclass jc, jint nCommand,
                                                       jbyte *buf) {
    // объявлены принудительно, передавались в качестве параметров
    int32_t *a1, a2, a3, a4;
    ...
    // отключено принудительно, теперь передаётся в качестве параметра
...
jstring Java_android_hardware_barcode_Scanner_ReadSCA(JNIEnv *env, jclass jc, jint nCode) {
    // обявлены принудительно, передавались в качестве параметров
    int a1, a2, a3;
...
jstring Java_android_hardware_barcode_Scanner_ReadSCAAuto(JNIEnv *env, jclass jc) {
    int a1; // объявлена принудительно, после декомпиляции якобы предавалось в качестве параметра функции
    int v1; // r5@1
    int v2; // r0@5
    int v3; // r6@7
    int v4; // r6@16
...
Java_android_hardware_barcode_Scanner_ReadSCAEx(JNIEnv *env, jclass jc, jint nCommand, jint nCode) {
    // объявлены принудительно, передавались в качестве праметров
    int a1, a2, a3;
    uint8_t a4;
...
jbyte *Java_android_hardware_barcode_Scanner_ReadData(JNIEnv *env, jclass jc, jint nCommand) {
    // объявлены принудительно, передавались как параметры
    int a1, a2, a3;
