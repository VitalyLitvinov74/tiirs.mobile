            ...
            public void onItemSelected(AdapterView<?> parentView, View selectedItemView, int position, long id)
            {
        		...
        		switch (position){
                    ...
                    case 2:
                        ...
                        break;
                    // TODO add this
                    //<item>Степени важности</item>
                    //<item>Типы измерений</item>
                    //<item>Результаты операций</item>
                    //<item>Типы операций</item>
                    //<item>Статусы нарядов</item>
                    default:
                        break;
                }
            }
            	...
            	if (current_ref==0)
            		{
            		 ...
            		 if (Spinner_addict.getSelectedItemId()>0)
            			 critical_type=criticalTypeDBAdapter.getUUIDByName(Spinner_addict.getSelectedItem().toString().substring(12, 13));
         			 Toast.makeText(getActivity(), type+" | "+critical_type, Toast.LENGTH_SHORT).show();
        	...
        	if (current_ref==0)
        		{
        		 if (Spinner_type.getSelectedItemId()>0)
        			 type=eqTypeDBAdapter.getUUIDByName(Spinner_type.getSelectedItem().toString());
        		 if (Spinner_addict.getSelectedItemId()>0)
        			 critical_type=criticalTypeDBAdapter.getUUIDByName("4");
     			 //Toast.makeText(getActivity(), type + " | "+critical_type + " " + Spinner_addict.getSelectedItem().toString(), Toast.LENGTH_SHORT).show();
        		 FillListViewEquipment(type,critical_type);
        		}
        	if (current_ref==1)
        		{
