...
public class TokenDBAdapter {
	...
	public String[] mColumns = {
			FIELD_TOKEN_TYPE_NAME,
			FIELD_ACCESS_TOKEN_NAME,
			FIELD_EXPIRES_IN_NAME,
			FIELD_USER_NAME_NAME,
			FIELD_ISSUED_NAME,
			FIELD_EXPIRES_NAME};
	...
	private Token getTokenData(Cursor cursor) {
		Token token = null;
		token = new Token(cursor.getString(cursor.getColumnIndex(FIELD_TOKEN_TYPE_NAME)),
	...
	public Token getTokenByUserName(String userName) {
		...
		Token token = null;
		Cursor cursor;
		cursor = mDb.query(TABLE_NAME, mColumns, FIELD_USER_NAME_NAME + "=?", new String[]{userName}, null, null, null);
	...
	/**
	 * <p>Добавляет/изменяет запись в таблице token</p>
	 * @param token_type
	 * @param access_token
	 * @param expires_in
	 * @param userName
	 ...
	 */
	public long replace(String token_type, String access_token, int expires_in, String userName, String issued, String expires) {
		...
		values.put(FIELD_ISSUED_NAME, issued);
		values.put(FIELD_EXPIRES_NAME, expires);
	...
	public long replace(Token token) {
		return replace(token.getToken_type(), token.getAccess_token(), token.getExpires_in(), token.getUserName(), token.getIssued(), token.getExpires());
	}
}
