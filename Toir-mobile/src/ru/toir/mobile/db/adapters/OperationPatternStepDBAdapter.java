...
public class OperationPatternStepDBAdapter extends BaseDBAdapter {
	...
	public static final class Projection {
		...
		public static final String DESCRIPTION = TABLE_NAME + '_'
				+ FIELD_DESCRIPTION;
		public static final String IMAGE = TABLE_NAME + '_' + FIELD_IMAGE;
		public static final String FIRST_STEP = TABLE_NAME + '_'
				+ FIELD_FIRST_STEP;
		public static final String LAST_STEP = TABLE_NAME + '_'
				+ FIELD_LAST_STEP;
	...
	static {
		...
		mProjection.put(Projection.DESCRIPTION,
				getFullName(TABLE_NAME, FIELD_DESCRIPTION) + " AS "
						+ Projection.DESCRIPTION);
		mProjection.put(Projection.IMAGE, getFullName(TABLE_NAME, FIELD_IMAGE)
				+ " AS " + Projection.IMAGE);
		mProjection.put(Projection.FIRST_STEP,
				getFullName(TABLE_NAME, FIELD_FIRST_STEP) + " AS "
						+ Projection.FIRST_STEP);
		mProjection.put(Projection.LAST_STEP,
				getFullName(TABLE_NAME, FIELD_LAST_STEP) + " AS "
	...
	public OperationPatternStep getItem(Cursor cursor) {
		...
		patternStep.setOperation_pattern_uuid(cursor.getString(cursor
				.getColumnIndex(FIELD_OPERATION_PATTERN_UUID)));
		patternStep.setDescription(cursor.getString(cursor
				.getColumnIndex(FIELD_DESCRIPTION)));
		patternStep.setImage(cursor.getString(cursor
				.getColumnIndex(FIELD_IMAGE)));
		patternStep.setFirst_step(cursor.getInt(cursor
				.getColumnIndex(FIELD_FIRST_STEP)) == 1);
		patternStep.setLast_step(cursor.getInt(cursor
				.getColumnIndex(FIELD_LAST_STEP)) == 1);
		patternStep.setTitle(cursor.getString(cursor
	...
	public ArrayList<OperationPatternStep> getItems(String pattern_uuid) {
		...
		Cursor cursor;
		cursor = mDb.query(TABLE_NAME, mColumns, FIELD_OPERATION_PATTERN_UUID
				+ "=?", new String[] { pattern_uuid }, null, null, null);
		if (cursor.moveToFirst()) {
