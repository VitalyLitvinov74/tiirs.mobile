	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tv.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				count++;
				if (count == 7) {
					// TODO реализовать изменение статусов нарядов
					ToirDatabaseContext databaseContext = new ToirDatabaseContext(
							mContext);
					...
					db.beginTransaction();
					for (Task task : tasks) {
						task.setTask_status_uuid(TaskStatusDBAdapter.Status.IN_WORK);
					...
					for (EquipmentOperation operation : operations) {
						operation.setOperation_status_uuid(OperationStatusDBAdapter.Status.NEW);
