...
public class TaskDBAdapter extends BaseDBAdapter {
	...
	public static final class Projection {
		...
		public static final String USER_UUID = TABLE_NAME + '_'
				+ FIELD_USER_UUID;
		public static final String CLOSE_DATE = TABLE_NAME + '_'
				+ FIELD_CLOSE_DATE;
		public static final String TASK_STATUS_UUID = TABLE_NAME + '_'
				+ FIELD_TASK_STATUS_UUID;
		...
		public static final String TASK_NAME = TABLE_NAME + '_'
				+ FIELD_TASK_NAME;
	...
	static {
		...
		mProjection.put(Projection.USER_UUID,
				getFullName(TABLE_NAME, FIELD_USER_UUID) + " AS "
						+ Projection.USER_UUID);
		mProjection.put(Projection.CLOSE_DATE,
				getFullName(TABLE_NAME, FIELD_CLOSE_DATE) + " AS "
						+ Projection.CLOSE_DATE);
		mProjection.put(Projection.TASK_STATUS_UUID,
				getFullName(TABLE_NAME, FIELD_TASK_STATUS_UUID) + " AS "
						+ Projection.TASK_STATUS_UUID);
		...
		mProjection.put(Projection.UPDATED,
				getFullName(TABLE_NAME, FIELD_UPDATED) + " AS "
						+ Projection.UPDATED);
		mProjection.put(Projection.TASK_NAME,
				getFullName(TABLE_NAME, FIELD_TASK_NAME) + " AS "
						+ Projection.TASK_NAME);
	}
	...
	/**
	 * Возвращает список всех нарядов Возможно временная функция
	 * 
	 * @param uuid
	 * @return
	 */
	...
	public ArrayList<Task> getTaskByUserAndStatus(String user_uuid,
		...
		cursor = mDb.query(TABLE_NAME, mColumns, FIELD_USER_UUID + "=? AND "
				+ FIELD_TASK_STATUS_UUID + "=?", new String[] { user_uuid,
				status_uuid }, null, null, null);
		if (cursor.moveToFirst()) {
	...
	/**
	 * <p>
	 * Возвращает дату завершения наряда
	 * </p>
	 * 
	 * @param uuid
	...
	/**
	 * <p>
	 * Возвращает дату создания наряда
	 * </p>
	 * 
	 * @param uuid
	 * @return format create date
	...
	public Task getItem(Cursor cursor) {
		...
		getItem(cursor, item);
		item.setUsers_uuid(cursor.getString(cursor
				.getColumnIndex(FIELD_USER_UUID)));
		item.setClose_date(cursor.getLong(cursor
				.getColumnIndex(FIELD_CLOSE_DATE)));
		item.setTask_status_uuid(cursor.getString(cursor
				.getColumnIndex(FIELD_TASK_STATUS_UUID)));
		item.setAttempt_send_date(cursor.getLong(cursor
	...
	public ArrayList<Task> getTaskByUserAndUpdated(String uuid) {
		...
		Cursor cursor;
		cursor = mDb
				.query(TABLE_NAME, mColumns, FIELD_USER_UUID + "=? AND "
						+ FIELD_UPDATED + "=1", new String[] { uuid }, null,
						null, null);
	...
	public Task getTaskByUuidAndUpdated(String uuid) {
		Cursor cursor;
		cursor = mDb
				.query(TABLE_NAME, mColumns, FIELD_UUID + "=? AND "
						+ FIELD_UPDATED + "=1", new String[] { uuid }, null,
						null, null);
	...
	public Cursor getTaskWithInfo(String userUuid, String statusUuid,
		...
		queryBuilder
				.setTables(TABLE_NAME
						+ ' '
						+ getLeftJoinTables(TABLE_NAME,
								TaskStatusDBAdapter.TABLE_NAME,
								FIELD_TASK_STATUS_UUID,
								TaskStatusDBAdapter.FIELD_UUID));
