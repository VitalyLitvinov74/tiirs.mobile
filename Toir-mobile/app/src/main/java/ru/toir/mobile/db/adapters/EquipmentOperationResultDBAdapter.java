 ...
 * @author olejek
 *         <p>
 *         Класс адаптер для таблицы equipment_operation_result
 *         </p>
 */
public class EquipmentOperationResultDBAdapter extends BaseDBAdapter {
    ...
    public static final String FIELD_EQUIPMENT_OPERATION_UUID =
            "equipment_operation_uuid";
    public static final String FIELD_START_DATE = "start_date";
    public static final String FIELD_END_DATE = "end_date";
    public static final String FIELD_OPERATION_RESULT_UUID =
            "operation_result_uuid";
    ...
    static {
        ...
        mProjection.put(Projection.EQUIPMENT_OPERATION_UUID,
                getFullName(TABLE_NAME, FIELD_EQUIPMENT_OPERATION_UUID)
                        + " AS " + Projection.EQUIPMENT_OPERATION_UUID);
        mProjection.put(Projection.START_DATE,
                getFullName(TABLE_NAME, FIELD_START_DATE) + " AS "
                        + Projection.START_DATE);
        mProjection.put(Projection.END_DATE,
                getFullName(TABLE_NAME, FIELD_END_DATE) + " AS "
                        + Projection.END_DATE);
        mProjection.put(Projection.OPERATION_RESULT_UUID,
                getFullName(TABLE_NAME, FIELD_OPERATION_RESULT_UUID) + " AS "
                        + Projection.OPERATION_RESULT_UUID);
        mProjection.put(Projection.TYPE, getFullName(TABLE_NAME, FIELD_TYPE)
                + " AS " + Projection.TYPE);
    ...
    /**
     * <p>
     * Возвращает по uuid дату начала выполнения операции
     * </p>
     *
    ...
    public Long getStartDateByUUID(String uuid) {
        ...
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            date = cursor.getLong(cursor.getColumnIndex(FIELD_START_DATE));
        } else {
            date = 0L;
        }
    ...
    /**
     * <p>
     * Временная функция Возвращает по uuid дату начала выполнения операции
     * </p>
     *
     * @param uuid    UUID операции
     */
    ...
    /**
     * <p>
     * Возвращает по uuid дату завершения операции
     * </p>
     *
     * @param uuid    UUID операции
     */
    public Long getEndDateByUUID(String uuid) {
        ...
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            date = cursor.getLong(cursor.getColumnIndex(FIELD_END_DATE));
        } else {
            date = 0L;
        }
    ...
    /**
     * <p>
     * Временная функция Возвращает по uuid, uuid (статуса) результата
     * выполнения операции
     * </p>
     *
     * @param uuid    UUID операции
    ...
    public String getOperationResultByUUID(String uuid) {
        ...
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            statusUuid = cursor.getString(cursor
                    .getColumnIndex(FIELD_OPERATION_RESULT_UUID));
        } else {
            statusUuid = "неизвестна";
        }
    ...
    /**
     * <p>
     * Возвращает тип по uuid (статуса) результата выполнения операции
     * </p>
     *
     * @param uuid    UUID операции
     */
    public int getOperationResultTypeByUUID(String uuid) {
        ...
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            type =  cursor.getInt(cursor.getColumnIndex(FIELD_TYPE));
        }
        // unknown type
        else {
            type = 3;
    ...
    public String getOperationResultByPartOfUUID(String uuid) {
        ...
        if (cursor.getCount() > 0) {
            cursor.moveToFirst();
            resultUuid = cursor.getString(cursor
                    .getColumnIndex(FIELD_OPERATION_RESULT_UUID));
        } else {
            resultUuid = "неизвестна";
        }
    ...
    public EquipmentOperationResult getItemByOperation(String operationUuid) {
        Cursor cursor;
        cursor = mDb.query(TABLE_NAME, mColumns, FIELD_EQUIPMENT_OPERATION_UUID
                + "=?", new String[]{operationUuid}, null, null, null);
        if (cursor.moveToFirst()) {
            return getItem(cursor);
        } else {
            return null;
     ...
     * </p>
     *
     * @param cursor Курсор с которым работает метод.
    ...
    public EquipmentOperationResult getItem(Cursor cursor) {
        ...
        result.setEquipment_operation_uuid(cursor.getString(cursor
                .getColumnIndex(FIELD_EQUIPMENT_OPERATION_UUID)));
        result.setStart_date(cursor.getLong(cursor
                .getColumnIndex(FIELD_START_DATE)));
        result.setEnd_date(cursor.getLong(cursor.getColumnIndex
                (FIELD_END_DATE)));
        result.setOperation_result_uuid(cursor.getString(cursor
                .getColumnIndex(FIELD_OPERATION_RESULT_UUID)));
        result.setType(cursor.getInt(cursor.getColumnIndex(FIELD_TYPE)));
        result.setAttempt_send_date(cursor.getLong(cursor
                .getColumnIndex(FIELD_ATTEMPT_SEND_DATE)));
        result.setAttempt_count(cursor.getInt(cursor
                .getColumnIndex(FIELD_ATTEMPT_COUNT)));
        result.setUpdated(cursor.getInt(cursor.getColumnIndex(FIELD_UPDATED))
                == 0);
        return result;
    }
     ...
     * </p>
     *
     * @param item Элемент таблицы для замены/добавления записи.
     * @return long id столбца или -1 если не удалось добавить запись
     */
    public long replace(EquipmentOperationResult item) {
        long id;
        ContentValues values = putCommonFields(item);

        values.put(FIELD_EQUIPMENT_OPERATION_UUID,
                item.getEquipment_operation_uuid());
        values.put(FIELD_START_DATE, item.getStart_date());
        values.put(FIELD_END_DATE, item.getEnd_date());
        values.put(FIELD_OPERATION_RESULT_UUID, item.getOperation_result_uuid
                ());
        ...
        values.put(FIELD_UPDATED, item.isUpdated() ? 1 : 0);
    ...
    public long delete(String operationUuid) {
        return mDb.delete(TABLE_NAME, FIELD_EQUIPMENT_OPERATION_UUID + "=?", new String[]{operationUuid});
    }
    ...
    public static final class Projection {
        ...
        public static final String EQUIPMENT_OPERATION_UUID = TABLE_NAME + '_'
        ...
        public static final String OPERATION_RESULT_UUID = TABLE_NAME + '_'
