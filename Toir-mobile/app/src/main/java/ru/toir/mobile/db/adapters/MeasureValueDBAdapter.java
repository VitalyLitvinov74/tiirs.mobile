...
public class MeasureValueDBAdapter extends BaseDBAdapter {
	...
	public static final class Projection {
		...
		public static final String OPERATION_PATTERN_STEP_RESULT_UUID = TABLE_NAME
				+ '_' + FIELD_OPERATION_PATTERN_STEP_RESULT_UUID;
	...
	static {
		...
		mProjection.put(
				Projection.OPERATION_PATTERN_STEP_RESULT_UUID,
				getFullName(TABLE_NAME,
						FIELD_OPERATION_PATTERN_STEP_RESULT_UUID)
						+ " AS "
						+ Projection.OPERATION_PATTERN_STEP_RESULT_UUID);
		mProjection.put(Projection.VALUE, getFullName(TABLE_NAME, FIELD_VALUE)
				+ " AS " + Projection.VALUE);
	...
	/**
	 * <p>
	 * Возвращает объект MeasureValue
	 * </p>
	 * 
	 * @param cursor
	 * @return
	...
	public MeasureValue getItem(Cursor cursor) {
		...
		getItem(cursor, item);
		item.setEquipment_operation_uuid(cursor.getString(cursor
				.getColumnIndex(FIELD_EQUIPMENT_OPERATION_UUID)));
		item.setOperation_pattern_step_result_uuid(cursor.getString(cursor
				.getColumnIndex(FIELD_OPERATION_PATTERN_STEP_RESULT_UUID)));
	...
	public long replace(MeasureValue item) {
		...
		values.put(FIELD_OPERATION_PATTERN_STEP_RESULT_UUID,
	...
	public MeasureValue getItem(String operationUuid, String resultUuid) {
		Cursor cursor;
		cursor = mDb.query(TABLE_NAME, mColumns, FIELD_EQUIPMENT_OPERATION_UUID
				+ "=? AND " + FIELD_OPERATION_PATTERN_STEP_RESULT_UUID + "=?",
				new String[] { operationUuid, resultUuid }, null, null, null);
		if (cursor.moveToFirst()) {
			return getItem(cursor);
		} else {
			return null;
