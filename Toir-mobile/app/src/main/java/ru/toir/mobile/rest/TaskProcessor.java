...
public class TaskProcessor {
    ...
    public Bundle GetTask(Bundle bundle) {
        ...
        String status = bundle.getString(TaskServiceProvider.Methods.PARAMETER_GET_TASK_STATUS);
...
//        SQLiteDatabase db = DatabaseHelper.getInstance(mContext).getWritableDatabase();
        ...
        // получаем шаблоны
        ArrayList<String> operationPatternUuids = new ArrayList<>(patternUuids);
        ...
        Bundle operationResultsResult = getOperationResults(operationTypeUuids.toArray(new String[]{""}));
        ...
        Bundle measureImageFileResult = getMeasureValueImages(measureValuesImages.toArray(new String[]{""}));
...
//                ArrayList<TaskSrv> serverTasks = gson.fromJson(jsonString, new TypeToken<ArrayList<TaskSrv>>() {}.getType());
//                if (serverTasks != null) {
//                    // разбираем и сохраняем полученные данные
//                    return saveTasks(serverTasks);
//                } else {
//                    result.putBoolean(IServiceProvider.RESULT, false);
//                    result.putString(IServiceProvider.MESSAGE, "Ошибка разбора ответа сервера на запрос нарядов.");
//                    return result;
//                }
                return null;
            } else {
                throw new Exception("Не удалось получить наряды. RESPONSE STATUS = " + response.mStatus);
            }
        } catch (Exception e) {
            e.printStackTrace();
            result.putBoolean(IServiceProvider.RESULT, false);
     ...
     * справочников.
     *
     * @param uuids список uuid шаблонов которые нужно получить
     * @return Bundle
     */
    private Bundle getPatterns(ArrayList<String> uuids) {
        ...
        try {
            ReferenceProcessor referenceProcessor = new ReferenceProcessor(mContext);
            Bundle extra = new Bundle();
            extra.putStringArrayList(ReferenceServiceProvider.Methods.GET_OPERATION_PATTERN_PARAMETER_UUID, uuids);
//            return referenceProcessor.getOperationPattern(extra);
     ...
     *
     * @param uuids список uuid типов операций для которых нужно получить данные
     * @return Bundle
     */
    private Bundle getOperationResults(String[] uuids) {
        ...
        try {
            ...
            bundle.putStringArray(ReferenceServiceProvider.Methods.GET_OPERATION_RESULT_PARAMETER_UUID, uuids);
//            return processor.getOperationResult(bundle);
    ...
    private Bundle getMeasureValueImages(String[] uuids) {
        ...
        try {
            ReferenceProcessor processor = new ReferenceProcessor(mContext);
            Bundle bundle = new Bundle();
            bundle.putStringArray(ReferenceServiceProvider.Methods.GET_IMAGE_FILE_PARAMETER_UUID, uuids);
//            return processor.getMeasureValueFile(bundle);
     ...
     *
     * @param tasks Список нарядов в серверном представлении
     * @return Bundle
     */
...
//            if (item.getOrderStatus().getId().toUpperCase(Locale.ENGLISH).equals(TaskStatusDBAdapter.Status.NEW.toUpperCase(Locale.ENGLISH))) {
...
//        result.putInt(TaskServiceProvider.Methods.RESULT_GET_TASK_COUNT, localTasks.size());
//
...
//            result.putBoolean(IServiceProvider.RESULT, false);
//            result.putString(IServiceProvider.MESSAGE, "Ошибка при сохранении нарядов.");
//            return result;
//        }
//
...
//            result.putBoolean(IServiceProvider.RESULT, false);
//            result.putString(IServiceProvider.MESSAGE, "Ошибка при сохранении статусов нарядов.");
//            return result;
//        }
//
//        // сохраняем операции
//        EquipmentOperationDBAdapter operationDBAdapter = new EquipmentOperationDBAdapter(new ToirDatabaseContext(mContext));
//        if (!operationDBAdapter.saveItems(TaskSrv.getEquipmentOperations(tasks))) {
//            result.putBoolean(IServiceProvider.RESULT, false);
//            result.putString(IServiceProvider.MESSAGE, "Ошибка при сохранении операций.");
//            return result;
//        }
//
...
//        if (!equipmentDBAdapter.saveItems(EquipmentOperationSrv.getEquipments(operations))) {
//            result.putBoolean(IServiceProvider.RESULT, false);
//            result.putString(IServiceProvider.MESSAGE, "Ошибка при сохранении оборудования.");
//            return result;
//        }
//
...
//        if (!operationTypeDBAdapter.saveItems(EquipmentOperationSrv.getOperationTypes(operations))) {
//            result.putBoolean(IServiceProvider.RESULT, false);
//            result.putString(IServiceProvider.MESSAGE, "Ошибка при сохранении типов операций.");
//            return result;
//        }
//
...
//        if (!operationStatusDBAdapter.saveItems(EquipmentOperationSrv.getOperationStatuses(operations))) {
//            result.putBoolean(IServiceProvider.RESULT, false);
//            result.putString(IServiceProvider.MESSAGE, "Ошибка при сохранении статусов операций.");
//            return result;
//        }
//
...
//		 */
//        operationTypeUuids = EquipmentOperationSrv.getOperationTypeUuids(operations);
//
...
//        if (!equipmentTypeDBAdapter.saveItems(EquipmentSrv.getEquipmentTypes(equipments))) {
//            result.putBoolean(IServiceProvider.RESULT, false);
//            result.putString(IServiceProvider.MESSAGE, "Ошибка при сохранении типов оборудования.");
//            return result;
//        }
//
...
//        if (!criticalTypeDBAdapter.saveItems(EquipmentSrv.getCriticalTypes(equipments))) {
//            result.putBoolean(IServiceProvider.RESULT, false);
//            result.putString(IServiceProvider.MESSAGE, "Ошибка при сохранении типов критичности оборудования.");
//            return result;
//        }
//
...
//        if (!equipmentStatusDBAdapter.saveItems(EquipmentSrv.getEquipmentStatuses(equipments))) {
//            result.putBoolean(IServiceProvider.RESULT, false);
//            result.putString(IServiceProvider.MESSAGE, "Ошибка при сохранении статусов оборудования.");
//            return result;
//        }
//
//        // получаем список документов для обязательной загрузки, позже загрузить
//        ArrayList<EquipmentDocumentation> doclist = EquipmentSrv.getEquipmentDocumentations(equipments);
...
//            result.putBoolean(IServiceProvider.RESULT, false);
//            result.putString(IServiceProvider.MESSAGE, "Ошибка при сохранении документации.");
//            return result;
//        }
//
...
//        if (!documentationTypeDBAdapter.saveItems(EquipmentSrv.getDocumentationTypes(equipments))) {
//            result.putBoolean(IServiceProvider.RESULT, false);
...
//                // базу
//                if (!operationSrv.getEquipmentOperationResultId().equals(BaseDBAdapter.uuidNull)) {
//                    EquipmentOperationResult operationResult = new EquipmentOperationResult();
//                    operationResult.setUuid(operationSrv.getEquipmentOperationResultId());
//                    operationResult.setEquipment_operation_uuid(operationSrv.getId());
//                    operationResult.setStart_date(operationSrv.getInspectionStartTime().getTime());
//                    operationResult.setEnd_date(operationSrv.getInspectionEndTime().getTime());
//                    operationResult.setOperation_result_uuid(operationSrv.getOperationResultId());
//                    // TODO не передаётся с сервера
//                    operationResult.setType(-1);
//                    operationResult.setCreatedAt(operationSrv.getCreatedAtTime());
//                    operationResult.setChangedAt(operationSrv.getChangedAtTime());
...
//        ArrayList<MeasureValue> measureValues = EquipmentOperationSrv.getMeasureValues(tasks);
//        if (!measureValueDBAdapter.saveItems(measureValues)) {
//            result.putBoolean(IServiceProvider.RESULT, false);
//            result.putString(IServiceProvider.MESSAGE, "Ошибка при сохранении результатов измерений.");
//            return result;
//        }
//
...
//        String[] taskUuids = bundle.getStringArray(TaskServiceProvider.Methods.PARAMETER_TASK_UUID);
//
...
//                    result = new Bundle();
//                    result.putBoolean(IServiceProvider.RESULT, false);
//                    result.putString(IServiceProvider.MESSAGE, "Ошибка при чтении результатов выполнения наряда.");
//                    return result;
//                }
//            }
//        }
...
//                        .setPrettyPrinting()
//                        .registerTypeAdapter(TaskRes.class, new TaskSerializer())
//                        .registerTypeAdapter(EquipmentOperationRes.class, new EquipmentOperationSerializer())
//                        .registerTypeAdapter(EquipmentOperationResultRes.class, new EquipmentOperationResultSerializer())
//                        .registerTypeAdapter(MeasureValueRes.class, new MeasureValueSerializer()).create();
//
...
//                } else {
//                    throw new Exception("Не удалось отправить результаты. RESPONSE STATUS = " + response.mStatus);
//                }
//            }
//
//        } catch (Exception e) {
...
//        TaskDBAdapter taskAdapter = new TaskDBAdapter(new ToirDatabaseContext(mContext));
//        EquipmentOperationDBAdapter operationAdapter = new EquipmentOperationDBAdapter(new ToirDatabaseContext(mContext));
//        EquipmentOperationResultDBAdapter operationResultAdapter = new EquipmentOperationResultDBAdapter(new ToirDatabaseContext(mContext));
//        MeasureValueDBAdapter valueAdapter = new MeasureValueDBAdapter(new ToirDatabaseContext(mContext));
//
//        SQLiteDatabase db = DatabaseHelper.getInstance(mContext).getWritableDatabase();
//        db.beginTransaction();
...
//
//            ArrayList<EquipmentOperationRes> operations = task.getEquipmentOperations();
...
//
//                ArrayList<MeasureValueRes> values = operation.getMeasureValues();
//                for (MeasureValueRes value : values) {
//                    value.setAttempt_count(value.getAttempt_count() + 1);
//                    valueAdapter.replace(value);
//                }
//
//                EquipmentOperationResultRes result = operation.getEquipmentOperationResult();
