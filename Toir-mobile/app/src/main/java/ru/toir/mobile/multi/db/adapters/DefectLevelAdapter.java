...
public class DefectLevelAdapter extends RealmBaseAdapter<DefectLevel> implements ListAdapter {
    public static final String TABLE_NAME = "DefectLevel";

    public DefectLevelAdapter(RealmResults<DefectLevel> data) {
        super(data);
    }

    @Override
    public DefectLevel getItem(int position) {
        DefectLevel defectLevel = null;
        if (adapterData != null) {
            defectLevel = adapterData.get(position);
        }
        return defectLevel;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (adapterData != null && viewHolder.title != null && adapterData.size() > 0) {
            defectLevel = adapterData.get(position);
            if (defectLevel != null)
                viewHolder.title.setText(defectLevel.getTitle());
        }
        ...
        if (convertView == null) {
            ...
            if (adapterData != null) {
                defectLevel = adapterData.get(position);
                if (defectLevel != null) {
                    textView.setText(defectLevel.getTitle());
                }
